...
```
 srun --gpus 1 --nodes 1  --export HF_ACCESS_TOKEN=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 --mem-per-cpu 12GB  --constraint 48BGgpu --ntasks-per-node 1 --cpus-per-gpu 10 /nlp/data/artemisp/mambaforge/envs/test_me/bin/python  src/pl_ft.py --cfg /nlp/data/artemisp/multigpu-lm-templates/src/configs/train/llama_mrqa.py
```
...
` srun --gpus 1 --nodes 1  --export  --mem-per-cpu 12GB  --constraint 48GBgpu --ntasks-per-node 1 --cpus-per-gpu 10 python  src/pl_ft.py --cfg /path/to/your/config`
